...
String? serializeParam(
  ...
  try {
    ...
    switch (paramType) {
      ...
      case ParamType.dynamic:
        data = _serializedynamic(param as dynamic);
      case ParamType.Document:
        final reference = (param as dynamic).reference;
        data = _serializedynamic(reference);
...
dynamic deserializeParam<T>(
  ...
  try {
    ...
    switch (paramType) {
      ...
      case ParamType.dynamic:
        return _deserializedynamic(param, collectionNamePath ?? []);
...
Future<dynamic> Function(String) getDoc(
  ...
  return (String ids) => _deserializedynamic(ids, collectionNamePath)
      .get()
...
Future<List<T>> Function(String) getDocList<T>(
  ...
  return (String idsList) {
    ...
    return Future.wait(
      docIds.map(
        (ids) => _deserializedynamic(ids, collectionNamePath)
            .get()
            .then((s) => recordBuilder(s)),
      ),
    ).then((docs) => docs.where((d) => d != null).map((d) => d! as T).toList());
  };
}
