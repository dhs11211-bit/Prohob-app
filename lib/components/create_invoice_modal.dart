...
class _CreateInvoiceModalState extends State<CreateInvoiceModal> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _customers = custData is List ? custData : (custData is Map && custData['data'] is List ? custData['data'] : []);
          _jobs = jobsData is List ? jobsData : (jobsData is Map && jobsData['data'] is List ? jobsData['data'] : []);
          
          if (_selectedJobId != null && _selectedCustomerId == null) {
            final job = _jobs.firstWhere((j) => j['id'].toString() == _selectedJobId, orElse: () => null);
          ...
          if (_selectedCustomerId != null) {
            final exists = _customers.any((c) => c['id'].toString() == _selectedCustomerId);
            if (!exists) {
              _customers.add({
          ...
          if (_selectedJobId != null) {
            final exists = _jobs.any((j) => j['id'].toString() == _selectedJobId);
            if (!exists) {
              _jobs.add({
                'id': _selectedJobId,
                'title': 'Assigned Job',
  ...
  Future<void> _fetchJobDetailsAndItems(String jobId) async {
    ...
    try {
      ...
      if (mounted) {
        final jobData = res is Map<String, dynamic> ? (res['data'] ?? res) : res;
        if (jobData != null && jobData['details'] is List) {
          final List<dynamic> details = jobData['details'];
          setState(() {
            _lineItems = details.map((d) => {
              'description': d['service_item']?['name'] ?? d['description'] ?? 'Job Item',
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text("Attach to Job (Optional)", style: TextStyle(color: Colors.white70, fontSize: 13)),
                  const SizedBox(height: 6),
                  widget.initialJobId != null
                      ...
                      : Autocomplete<Map<String, dynamic>>(
                          displayStringForOption: (j) => "Job #${j['id']} - ${j['title'] ?? 'No Title'}",
                          optionsBuilder: (TextEditingValue textEditingValue) async {
                            final query = textEditingValue.text.trim();
                            if (query.isEmpty) {
                              return filteredJobs.map((e) => e as Map<String, dynamic>).toList();
                            }
                            
                            try {
                              final now = DateTime.now();
                              final startDate = now.subtract(const Duration(days: 10)).toIso8601String().split('T')[0];
                              final endDate = now.add(const Duration(days: 30)).toIso8601String().split('T')[0];
                              
                              final res = await ApiService.instance.get('/admin/jobs?search=$query&start_date=$startDate&end_date=$endDate');
                              final List<dynamic> data = res is List ? res : (res['data'] ?? []);
                              
                              if (mounted) {
                                setState(() {
                                  for (var job in data) {
                                    if (!_jobs.any((j) => j['id'].toString() == job['id'].toString())) {
                                      _jobs.add(job);
                                    }
                                  }
                                });
                          ...
                          onSelected: (selection) => _onJobSelected(selection['id'].toString()),
                          fieldViewBuilder: (context, textEditingController, focusNode, onFieldSubmitted) {
                            // Sync current selected job into the field
                            if (_selectedJobId != null && textEditingController.text.isEmpty) {
                               final j = _jobs.firstWhere((job) => job['id'].toString() == _selectedJobId, orElse: () => null);
                               if (j != null) {
                                 textEditingController.text = "Job #${j['id']} - ${j['title'] ?? 'No Title'}";
                               }
                            }
                            return TextFormField(
                              ...
                              validator: (val) {
                                if ((val != null && val.isNotEmpty) && _selectedJobId == null) {
                                ...
                                child: SizedBox(
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (BuildContext context, int index) {
                                      ...
                                      return ListTile(
                                        title: Text("Job #${j['id']} - ${j['title'] ?? 'No Title'}", style: const TextStyle(color: Colors.white, fontSize: 13)),
                                        onTap: () {
                                          onSelected(j);
                                        },
                                      );
...
void showCreateInvoiceModal(BuildContext context, {String? initialJobId, String? initialCustomerId, double? initialAmount, VoidCallback? onInvoiceCreated}) {
